...
if not (HAVE_SK and HAVE_PW):
    print('needs: pip install scikit-image (pipeline venv) and playwright (tools/node_modules).'); sys.exit(2)
rows=[]
with sync_playwright() as pw:
    ...
    for room in rooms:
        ...
        for line in mf.read_text().splitlines():
            ...
            page.goto(f"{HARNESS}?photo=/{GT.name}/{room.name}/{mk['photo']}&quad={json.dumps(mk['quad'])}&finish={mk['finish']}", wait_until='load')
            ...
            panels_est=max(1, math.ceil(math.ceil(mk['width_m']/0.29)*mk['height_m']/3.05))
            scale_err=abs(panels_est-mk['panels'])/max(1,mk['panels'])*100
            rows.append({'room':room.name,'photo':mk['photo'],'finish':mk['finish'],'ssim':round(float(s),3),'dE2000':round(float(de),2),'scale_err_pct':round(scale_err,1)})
    b.close()
